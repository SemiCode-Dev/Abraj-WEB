...
@section('title', __('Transfer Bookings'))

@section('content')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('Transfer Bookings') }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Manage transfer booking requests') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <form method="GET" action="{{ route('admin.transfer-bookings.index') }}" class="flex gap-4 items-end">
            <div class="flex-1">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Trip Type') }}</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-600 dark:text-gray-400">{{ $booking->trip_type === 'go' ? __('Go') : __('Go and Back') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-600 dark:text-gray-400">{{ $booking->transfer_date->format('M d, Y') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <form method="POST" action="{{ route('admin.transfer-bookings.update-status', ['locale' => app()->getLocale(), 'transferBooking' => $booking->id]) }}" class="inline-flex gap-2">
                                @csrf
                                @method('PATCH')
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-8 text-center text-gray-600 dark:text-gray-400">
                            <i class="fas fa-car text-3xl mb-2 text-gray-400 block"></i>
