...
<body class="font-cairo antialiased bg-gray-50 dark:bg-gray-900" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    <div class="min-h-screen flex">
        <!-- Sidebar -->
        <aside id="admin-sidebar" class="admin-sidebar fixed lg:fixed inset-y-0 {{ app()->getLocale() === 'ar' ? 'right-0' : 'left-0' }} z-30 w-72 transform transition-transform duration-300 ease-in-out {{ app()->getLocale() === 'ar' ? 'translate-x-full lg:translate-x-0' : '-translate-x-full lg:translate-x-0' }}">
            <div class="flex flex-col h-screen overflow-hidden">
                ...
                <div class="p-4 border-t border-white/10 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex-shrink-0">
                    <div class="relative">
                        <div id="sidebar-user-toggle" class="admin-profile-card rounded-xl p-4 cursor-pointer">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg flex-shrink-0">
                                    A
                                </div>
                ...
                <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @php
                            ...
                            $adminPath = str_replace(['ar/admin/', 'en/admin/'], '', $currentPath);
                            if (empty($adminPath) || $adminPath === 'admin') {
                                $adminPath = 'dashboard';
                            }
                            $targetLocale = app()->getLocale() === 'ar' ? 'en' : 'ar';
                            $targetUrl = url('/' . $targetLocale . '/admin/' . $adminPath);
                        @endphp
                        <a href="{{ $targetUrl }}" class="p-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition" title="{{ app()->getLocale() === 'ar' ? 'Switch to English' : 'التبديل إلى العربية' }}">
                            <i class="fas fa-globe text-xl"></i>
                        </a>

                        <!-- Theme Toggle -->
                        <button id="admin-theme-toggle" class="p-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition">
    ...
    <div id="sidebar-backdrop" class="fixed inset-0 bg-black bg-opacity-50 z-20 lg:hidden hidden transition-opacity"></div>
