            ...
            <form action="{{ route('flights.book', ['locale' => app()->getLocale()]) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" name="adults" value="{{ old('adults', 1) }}" required min="1" max="20"
                               placeholder="{{ __('Enter number of adults') }}"
                               class="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-gray-900 dark:text-gray-100">
                        @error('adults')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="children" value="{{ old('children', 0) }}" required min="0" max="20"
                               placeholder="0"
                               class="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-gray-900 dark:text-gray-100">
                        @error('children')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            if (countryId) {
                fetch(`/{{ app()->getLocale() }}/flights/cities/${countryId}`)
                    ...
                    .catch(error => {
                        ...
                        originCitySelect.innerHTML = '<option value="">{{ __('Error loading cities') }}</option>';
                    });
            } else {
                originCitySelect.innerHTML = '<option value="">{{ __('Select City') }}</option>';
            }
