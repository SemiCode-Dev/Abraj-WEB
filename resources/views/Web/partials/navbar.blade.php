    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            ...
            <div class="flex items-center {{ app()->getLocale() === 'ar' ? 'space-x-reverse space-x-4' : 'space-x-4' }}">
                ...
                <button id="theme-toggle" aria-label="Toggle theme" class="p-1.5 rounded-md text-gray-700 dark:text-gray-200 hover:text-orange-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                    <i id="theme-sun" class="fas fa-sun text-lg"></i>
                    <i id="theme-moon" class="fas fa-moon text-lg hidden"></i>
                </button>
                
                @auth
                    <!-- Profile Dropdown - Desktop -->
                    <div class="hidden md:block relative" id="profile-dropdown-container">
                        <button id="profile-toggle" class="relative w-10 h-10 rounded-full overflow-hidden border-2 border-orange-500 hover:border-orange-600 transition focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2">
                            @if(auth()->user()->avatar ?? false)
                                <img src="{{ auth()->user()->avatar }}" alt="{{ auth()->user()->name }}" class="w-full h-full object-cover">
                            @else
                                <div class="w-full h-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-semibold">
                        ...
                        <div id="profile-dropdown" class="absolute {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 opacity-0 invisible transition-all duration-200 z-50">
                            <a href="{{ route('profile', ['locale' => app()->getLocale()]) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                                <i class="fas fa-user {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('Profile') }}
                            </a>
                            <a href="{{ route('requests', ['locale' => app()->getLocale()]) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                    ...
                    <div class="md:hidden">
                        <a href="{{ route('profile', ['locale' => app()->getLocale()]) }}" class="text-gray-700 dark:text-gray-200 hover:text-orange-500 transition">
                            @if(auth()->user()->avatar ?? false)
                                <img src="{{ auth()->user()->avatar }}" alt="{{ auth()->user()->name }}" class="w-8 h-8 rounded-full border-2 border-orange-500">
                            @else
                ...
                @else
                    ...
                    <button onclick="openAuthModal()" class="hidden md:block bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 py-2 rounded-xl hover:from-orange-600 hover:to-orange-700 transition font-semibold shadow-lg">
                        {{ __('Login') }}
                    </button>
                @endauth
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                ...
                @auth
                    ...
                    <div class="pt-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
                        <a href="{{ route('profile', ['locale' => app()->getLocale()]) }}" onclick="closeMobileMenu()" class="block text-gray-700 dark:text-gray-200 hover:text-orange-500 font-medium py-2">
                            <i class="fas fa-user {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('Profile') }}
                        </a>
                        <a href="{{ route('requests', ['locale' => app()->getLocale()]) }}" onclick="closeMobileMenu()" class="block text-gray-700 dark:text-gray-200 hover:text-orange-500 font-medium py-2">
                            <i class="fas fa-list {{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }}"></i>{{ __('Requests') }}
                        </a>
                        <form method="POST" action="{{ route('logout', ['locale' => app()->getLocale()]) }}">
                            @csrf
