            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
                ...
                <div class="text-center mb-8">
                    <label for="imageInput" class="cursor-pointer inline-block">
                        @if ($user->image)
                            <img src="{{ asset('storage/' . $user->image) }}" alt="{{ $user->name }}" id="imagePreview"
                                class="w-32 h-32 rounded-full mx-auto border-4 border-orange-500 mb-4 object-cover hover:opacity-80 transition">
                        @else
                            <div id="imagePreview"
                                class="w-32 h-32 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white text-4xl font-bold mx-auto border-4 border-orange-500 mb-4 hover:opacity-80 transition cursor-pointer">
                ...
                <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6"
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {{ __('Maximum file size: 2MB. Allowed types: jpeg, png, jpg, gif') }}</p>
                    </div>

                    <!-- Password Section -->
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{{ __('Change Password') }}
                        </h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                            {{ __('Leave blank if you dont want to change your password') }}</p>
                    ...
                    <div class="flex gap-4 pt-6">
                        <button type="submit"
                            class="flex-1 px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl hover:from-orange-600 hover:to-orange-700 transition font-semibold shadow-lg">
                            {{ __('Update Profile') }}
                        </button>
                    </div>
                </form>
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    if (preview.tagName === 'IMG') {
                        ...
                        img.className =
                            'w-32 h-32 rounded-full mx-auto border-4 border-orange-500 mb-4 object-cover hover:opacity-80 transition';
